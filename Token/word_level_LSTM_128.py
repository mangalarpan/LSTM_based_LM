...
print('Build model...')
model = Sequential()
model.add(CuDNNLSTM(128,input_shape=(maxlen,EMBEDDING_DIM ),return_sequences=True))
...
def on_epoch_end(epoch, logs):
    ...
    path ="" + str(epoch)
    model.save_weights(path + 'lstm_word_weights_128.hdf5')
    

print_callback = LambdaCallback(on_epoch_end=on_epoch_end)
csv_logger = CSVLogger('training_word_128.log')
model.fit(x, y, batch_size=128, epochs=50,callbacks=[print_callback,csv_logger])
model.save('lstm_word_model_128.h5')
